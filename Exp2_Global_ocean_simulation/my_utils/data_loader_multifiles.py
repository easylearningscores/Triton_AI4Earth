...
class GetDataset(Dataset):
    ...
    def __getitem__(self, global_idx):
        ...
        if self.params.multi_steps_finetune == 1:
            ...
            climate_mean_ocean = self.climate_mean[(local_idx-self.dt*self.n_history):(local_idx+1):self.dt, self.ocean_channels, :120, :240]
            ocean = reshape_fields( 
                    self.files[year_idx][(local_idx-self.dt*self.n_history):(local_idx+1):self.dt, self.ocean_channels, :120, :240] - climate_mean_ocean, 
                    'ocean', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            climate_mean_force_future = self.climate_mean[local_idx+step, self.atmos_channels, :120, :240]
            force_future = reshape_fields( 
                    self.files[year_idx][local_idx+step, self.atmos_channels, :120, :240] - climate_mean_force_future, 
                    'force', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            climate_mean_tar = self.climate_mean[local_idx+step, self.out_channels, :120, :240]
            tar = reshape_fields(
                    self.files[year_idx][local_idx+step, self.out_channels, :120, :240] - climate_mean_tar, 
                    'tar', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            climate_mean_tar = self.climate_mean[local_idx+step:local_idx+step+self.params.multi_steps_finetune, self.in_channels, :120, :240]
            tar_data = self.files[year_idx][local_idx+step:local_idx+step+self.params.multi_steps_finetune, self.in_channels, :120, :240]
            tar = reshape_fields( 

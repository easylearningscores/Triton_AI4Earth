...
class Triton_finetune(nn.Module):
    def __init__(
        self,
        shape_in,
        ...
        out_time_seq_length=40
    ):
        super(Triton_finetune, self).__init__()
        T, C, H, W = shape_in
    ...
    def _single_forward(self, x_raw):
        B, T, C, H, W = x_raw.shape
        x = x_raw.view(B*T, C, H, W)
        
        embed, skip = self.atmospheric_encoder(x)
        _, C_, H_, W_ = embed.shape

        z = embed.view(B, T, C_, H_, W_)
        temporal_bias = z
        temporal_hidden = self.temporal_evolution(temporal_bias)
        hid = temporal_hidden.view(B*T, C_, H_, W_)

        Y = self.atmospheric_decoder(hid, skip)
        return Y.reshape(B, T, -1, H, W)

    def forward(self, input_sequence):

        base_pred = self._single_forward(input_sequence)
        
        if self.output_time_seq_length == self.input_time_seq_length:
            return base_pred
        if self.output_time_seq_length < self.input_time_seq_length:
            return base_pred[:, :self.output_time_seq_length]
        
        predictions = [base_pred]
        d = self.output_time_seq_length // self.input_time_seq_length
        m = self.output_time_seq_length % self.input_time_seq_length

        for _ in range(1, d):
            new_pred = self._single_forward(predictions[-1])
            predictions.append(new_pred)

        if m > 0:
            final_pred = self._single_forward(predictions[-1])[:, :m]
